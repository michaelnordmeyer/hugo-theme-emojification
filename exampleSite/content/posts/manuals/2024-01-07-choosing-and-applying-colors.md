...
categories: ["Manuals"]
url: manuals/choosing-and-applying-colors
params:
  icon: 🎨
  theme_color: hotpink
...
Or the whole site can have the same color declared in the site’s `hugo.toml`.
...
This page’s `hotpink.css`:

```css
@import "base/black-white.css";
...
To have a defined color in frontmatter's `theme_color` or `hugo.toml`’s `theme_color` be applied, a CSS file has to exist in `assets/assets/css`. Both are located in the theme itself, and not the demo you are using now.

### Named Colors

For named colors like “orange”, the file has to be named `orange.css`. Create a directory called `assets` in your site’s root directory, and a subdirectory called `assets` again, and finally a subdirectory called `css`. In there create a file called `orange.css`, and put the colors derived from hsl(39 100% 50%) in there.

### Hex Colors

For hex codes like `#c34cff`, the file has to be named `c34cff.css` without the preceding hash. That’s because Hugo excludes file starting with a hash from its build process. And because file names can be case-sensitive, the code should always be lower-case.
